...
export default function IndexContent({ user }: IndexContentProps) {
  ...
  const formatAIAdvice = (text: string) => {
    if (!text) return "";
    let formatted = text
    ...
    if (/<li>/.test(formatted)) formatted = formatted.replace(/(<li>.*<\/li>)/gis, "<ul>$1</ul>");
    ...
    <div style={{ padding: 20 }}>
      ...
      <h2
        style={{
          ...
          fontWeight: 500,        // normal weight (less bold)
          color: "#333333",       // optional: softer color
          marginBottom: "24px",   // optional spacing
        }}
      >
